...
- Local macOS, R 4.2.3
- GitHub Actions is configured to run `R CMD check --no-manual --as-cran` on macOS, Windows,
  Ubuntu, and Ubuntu R-devel.

## R CMD check results

- `R CMD check --no-manual agentr_0.2.8.tar.gz`: 0 errors, 0 warnings, 0 notes
- `R CMD check --no-manual --as-cran agentr_0.2.8.tar.gz`: 0 errors, 0 warnings, 2 notes
...
- Confirm the GitHub Actions `R CMD check --no-manual --as-cran` matrix passes on current R release
  and R-devel.
