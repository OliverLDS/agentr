...
Internally, `0.2.5` keeps `Scaffolder` as the facade while separating:
...
Within `0.2.5`, workflow ownership labels live in workflow metadata so workflow-first compatibility remains intact while agent designs can still mark which subsystem owns each node. Those ownership labels can now be edited incrementally rather than only replaced wholesale.
