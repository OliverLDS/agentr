test_that("complete AgentSpec fixture round-trips through explicit persistence", {
  ...
  expect_true(inherits(loaded$memory_spec, "MemorySpec"))
  expect_equal(length(loaded$memory_spec$fields), 4L)
  expect_equal(loaded$memory_spec$get_field("current_task_context")$memory_type, "context")
  expect_equal(loaded$memory_spec$get_field("approved_macro_concepts")$persistence, "cold_start_rds")
