...
Version `0.2.6.11` fixes branch-edge anchoring in process-layout review diagrams so conditional branches leave decision nodes from the correct side and enter branch targets at the top. `agentr` can package workflow graphs, memory schemas, narrative knowledge, graph knowledge, proposal states, and structured feedback schema into one offline review page while remaining the core reasoning and scaffolding layer, not the transport or execution layer.
