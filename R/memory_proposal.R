#' @keywords internal
.memory_proposal_statuses <- function() {
  c("pending", "under_discussion", "approved", "rejected", "superseded")
}

#' @keywords internal
.normalize_memory_proposal_status <- function(x) {
  match.arg(as.character(x)[1], choices = .memory_proposal_statuses())
}

#' @keywords internal
.as_memory_spec_object <- function(x) {
  if (inherits(x, "MemorySpec")) {
    x$validate()
    return(x)
  }
  if (is.list(x) && all(c("fields", "metadata") %in% names(x))) {
    spec <- MemorySpec$new(fields = x$fields, metadata = x$metadata)
    spec$validate()
    return(spec)
  }
  stop("Expected a `MemorySpec` object or MemorySpec list payload.", call. = FALSE)
}

#' Validate a memory proposal
#'
#' @param x Memory proposal record or object.
#'
#' @return The validated proposal, invisibly.
#' @export
validate_memory_proposal <- function(x) {
  if (inherits(x, "MemoryProposal")) {
    x <- x$to_list()
  }
  required <- c(
    "id", "memory_spec", "status", "notes", "history", "metadata",
    "created_at", "updated_at", "approved_at", "rejected_at",
    "superseded_by", "supersedes"
  )
  if (!is.list(x) || !all(required %in% names(x))) {
    stop("Memory proposal must contain the required proposal fields.", call. = FALSE)
  }
  if (!is.character(x$id) || length(x$id) != 1L || !nzchar(x$id)) {
    stop("Memory proposal `id` must be a non-empty string.", call. = FALSE)
  }
  .as_memory_spec_object(x$memory_spec)
  .normalize_memory_proposal_status(x$status)
  if (!is.list(x$history)) {
    stop("Memory proposal `history` must be a list.", call. = FALSE)
  }
...
#' @field memory_spec Proposed [`MemorySpec`].
...
#'
#' @param memory_spec Proposed [`MemorySpec`] or serializable memory-spec list.
...
#' @param ... Unused.
#' @export
MemoryProposal <- R6::R6Class(
  classname = "MemoryProposal",
  public = list(
    id = NULL,
    memory_spec = NULL,
    status = NULL,
    ...
    initialize = function(
      ...
      self$memory_spec <- .as_memory_spec_object(memory_spec)
      self$status <- .normalize_memory_proposal_status(status)
    ...
    transition = function(status, note = NULL, timestamp = Sys.time()) {
      status <- .normalize_memory_proposal_status(status)
      allowed <- list(
      ...
      if (!identical(self$status, status) && !(status %in% allowed[[self$status]])) {
        stop("Invalid memory proposal transition.", call. = FALSE)
      }
      self$status <- status
      self$updated_at <- as.POSIXct(timestamp)[1]
    ...
    to_list = function() {
      list(
        id = self$id,
        memory_spec = self$memory_spec$to_list(),
        status = self$status,
        notes = self$notes,
    ...
    print = function(...) {
      ...
      cat("Fields:", length(self$memory_spec$fields), "\n")
      invisible(self)
    }
  )
)
...
#' @keywords internal
.as_memory_proposal_object <- function(x) {
  if (inherits(x, "MemoryProposal")) {
    x$validate()
    return(x)
  }
  if (!is.list(x)) {
    stop("Memory proposals must be proposal records or `MemoryProposal` objects.", call. = FALSE)
  }
  MemoryProposal$new(
    id = x$id,
...
#'
#' @field approved_memory_spec Approved [`MemorySpec`].
#' @field proposals Named list of [`MemoryProposal`] objects.
#' @field history Proposal-state history.
#'
#' @param approved_memory_spec Approved [`MemorySpec`] or serializable memory-spec list.
#' @param proposals Initial proposals.
#' @param history Proposal-state history.
#' @param proposal [`MemoryProposal`] object or serializable proposal record.
#' @param proposal_id Proposal identifier.
#' @param status Optional status filter.
#' @param note Discussion or transition note.
#' @param source Discussion source.
#' @export
MemoryProposalState <- R6::R6Class(
  classname = "MemoryProposalState",
  public = list(
    approved_memory_spec = NULL,
    proposals = NULL,
    history = NULL,

    #' @description
    #' Create a memory proposal state container.
    initialize = function(approved_memory_spec = MemorySpec$new(), proposals = list(), history = list()) {
      self$approved_memory_spec <- .as_memory_spec_object(approved_memory_spec)
      self$proposals <- list()
      self$history <- history
    ...
    validate = function() {
      self$approved_memory_spec$validate()
    ...
    add_proposal = function(proposal) {
      proposal <- .as_memory_proposal_object(proposal)
    ...
    get_proposal = function(proposal_id) {
      ...
      if (is.null(proposal)) {
        stop("Unknown memory proposal: ", proposal_id, call. = FALSE)
      }
      proposal
    },
    ...
    approve_proposal = function(proposal_id, note = NULL) {
      ...
      self$approved_memory_spec <- proposal$memory_spec$clone(deep = TRUE)
      self$proposals[[proposal$id]] <- proposal
    ...
    as_list = function() {
      list(
        approved_memory_spec = self$approved_memory_spec$to_list(),
