...
design_review_html <- function(
  ...
  paste(
    ...
    ".grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:10px}.card{border:1px solid #d8d2c4;border-radius:8px;padding:10px;background:#fff}",
    ".card button{all:unset;cursor:pointer;display:block;width:100%}.id{font:12px ui-monospace,SFMono-Regular,Menlo,monospace;color:#6b7280}.label{font-weight:600}.small{font-size:12px;color:#52606d}",
    ...
    "function drawWorkflowGraph(el,nodes,edges){const w=760,h=Math.max(180,nodes.length*74);const yStep=h/(nodes.length+1);let svg=`<svg viewBox='0 0 ${w} ${h}' width='100%' height='${h}' role='img' aria-label='Workflow graph'>`;nodes.forEach((n,i)=>{n._x=80+(i%3)*245;n._y=yStep*(i+1);});edges.forEach(e=>{const a=nodes.find(n=>n.id===e.from),b=nodes.find(n=>n.id===e.to);if(a&&b){svg+=`<line x1='${a._x+130}' y1='${a._y}' x2='${b._x}' y2='${b._y}' stroke='#6b7280' stroke-width='1.5' marker-end='url(#arrow)'/>`;}});svg+=`<defs><marker id='arrow' viewBox='0 0 10 10' refX='8' refY='5' markerWidth='6' markerHeight='6' orient='auto-start-reverse'><path d='M 0 0 L 10 5 L 0 10 z' fill='#6b7280'/></marker></defs>`;nodes.forEach(n=>{svg+=`<g tabindex='0'><rect x='${n._x}' y='${n._y-24}' width='150' height='48' rx='8' fill='${n.human_required?'#fff1d6':'#e8f3ee'}' stroke='#607466'/><text x='${n._x+75}' y='${n._y-4}' text-anchor='middle' font-size='11'>${esc(n.id)}</text><text x='${n._x+75}' y='${n._y+13}' text-anchor='middle' font-size='12'>${esc(n.label).slice(0,24)}</text></g>`;});svg+='</svg>';el.innerHTML=svg;}",
